...
export function InlineExpressionMenu({
  ...
  return (
    <div ref={layerRef} className="absolute bottom-14 right-4 z-20 w-[min(360px,calc(100%-2rem))] rounded-lg bg-[#f7f4ed] p-3 text-[#1c1c1c] shadow-[0_16px_48px_rgba(28,28,28,0.14)]">
      ...
      {view === "library" ? (
        <div className="mt-3 space-y-2">
          ...
          />
          {recalled.length === 0 ? (
            <p className="rounded-md bg-black/[0.025] p-3 text-xs text-[#1c1c1c]/50">
              暂时还没有保存的表达。
            </p>
          ) : (
            recalled.map((item) => (
