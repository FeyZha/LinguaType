...
export function CommonIssuesPanel({ items, onDelete }: CommonIssuesPanelProps) {
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4">
      <h2 className="text-sm font-semibold text-slate-900">Common Issues</h2>
      {sorted.length === 0 ? (
        <p className="mt-3 text-sm text-slate-500">
          Apply sentence revisions to collect recurring correction patterns.
        </p>
      ) : (
