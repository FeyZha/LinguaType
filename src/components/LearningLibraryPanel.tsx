...
export function LearningLibraryPanel({
  ...
  async function exportJson() {
    ...
    setExportMessage("Library JSON copied.");
  }
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-slate-900">Learning Library</h2>
          <p className="mt-1 text-xs text-slate-500">Your reusable expression assets.</p>
        </div>
        <button
          type="button"
          onClick={exportJson}
      ...
      <div className="mt-3 grid gap-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <select
            ...
            className="rounded-md border border-slate-300 px-2 py-2 text-sm"
            aria-label="Filter by writing mode"
          >
      ...
      {visibleItems.length === 0 ? (
        <p className="mt-3 text-sm text-slate-500">Apply a revision to save useful expressions here.</p>
      ) : (
          ...
          {visibleItems.map((item) => (
            <li key={item.id} className="rounded-md bg-slate-50 p-3 text-sm">
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">
                ...
                {item.writingMode === writingMode ? <span>Current mode</span> : null}
              </div>
