...
- 侧栏品牌区使用主题对应的图片资产，展开时显示完整 wordmark，折叠时显示单字母 mark。
...
表达库支持搜索、筛选、收藏、复制、插入、删除和 JSON 导出。页面以统一小卡片网格展示表达资产，来源句默认截断，点击卡片可在原位展开完整来源。保存时会按 `type + normalized content` 去重，重复内容会累计使用次数并更新时间。
