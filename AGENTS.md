...
1. Process only the latest non-empty sentence for sentence enhancement.
...
Trigger settings are stored in `linguatype.triggerSettings.v1`.
...
Current keys: `linguatype.apiSettings.v1`, `linguatype.writingDraft.v1`, `linguatype.learningLibrary.v1`, `linguatype.correctionEvents.v1`, `linguatype.paragraphHealthCache.v1`, `linguatype.triggerSettings.v1`.
...
`/api/explain-selection` explains selected text only. It must not rewrite selected text, return replacement text, trigger latest sentence enhancement, or trigger learning extraction.
...
2. Do not save on Regenerate, Copy revised sentence, Cancel, Paragraph Health, Paragraph Flow, or Selection Explain.
...
The Inline Expression Menu is local by default.
...
Before considering a task complete, run `npm test`, `npm run lint`, and `npm run build`.
...
- Paragraph Health trigger modes and cache/throttle behavior
- Inline Expression Menu behavior
